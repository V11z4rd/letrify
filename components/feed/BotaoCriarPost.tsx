...
export default function BotaoCriarPost({ onPostCreated }: { onPostCreated?: () => void }) {
  ...
  useEffect(() => {
    // 💡 SÓ DISPARA A BUSCA SE O USUÁRIO ATIVAR O TOGGLE E A LISTA ESTIVER VAZIA
    if (isRecrutamento && gruposFiltrados.length === 0) {
  ...
  }, [isRecrutamento, gruposFiltrados.length]);
  ...
  return (
    ...
    >
      <form onSubmit={handlePublicar} className="flex flex-col gap-4">
        ...
        {/* ÁREA DO RECRUTAMENTO (Dropdown Filtrado no Client) */}
        {isRecrutamento && (
          <div 
