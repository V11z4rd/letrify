...
export default function EditorGrupo({ dadosIniciais, onClose, onSave }: EditorGrupoProps) {
  ...
  const [fotoCapa, setFotoCapa] = useState<string | File>(dadosIniciais.fotoCapa);
  const [previewCapa, setPreviewCapa] = useState("");
  const [salvando, setSalvando] = useState(false);
  
  const bannerInputRef = useRef<HTMLInputElement>(null);

  // Gerencia previews locais em blobs sem vazar memória
  useEffect(() => {
    let urlCapa = "";

    if (fotoCapa instanceof File) {
      urlCapa = URL.createObjectURL(fotoCapa);
      setPreviewCapa(urlCapa);
    } else {
      setPreviewCapa(fotoCapa || "");
    }
  ...
  const resetarParaOriginal = () => {
    setFotoCapa(dadosIniciais.fotoCapa);
  ...
  const handleConfirmar = async () => {
    ...
    setSalvando(true);
    try {
      // Mescla o formulário de texto com o estado limpo da foto capa para enviar ao page.tsx
      await onSave({
        ...formData,
        fotoCapa: fotoCapa
      });
    } catch (err) {
      console.error("Erro ao submeter alterações do grupo:", err);
    } finally {
      setSalvando(false);
    }
  };
        ...
        <div className="px-6 sm:px-8 pb-8 pt-6 relative space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative flex items-center">
                <input 
                  type="text"
                  value={fotoCapa instanceof File ? "📁 Arquivo de imagem selecionado localmente" : fotoCapa}
                  onChange={(e) => setFotoCapa(e.target.value)}
                  disabled={fotoCapa instanceof File}
                  placeholder="https://exemplo.com/sua-capa.jpg"
                  className={`w-full px-4 py-2.5 text-xs font-bold rounded-xl border outline-none transition-all focus:ring-1 focus:ring-[var(--cor-destaque)] ${fotoCapa instanceof File ? 'opacity-60 cursor-not-allowed select-none italic' : ''}`}
                  style={{ backgroundColor: 'var(--cor-fundo-app)', color: 'var(--cor-texto-principal)', borderColor: 'var(--cor-fundo-sidebar)' }}
                />
                
                {fotoCapa instanceof File && (
                  <button
                    type="button"
                    onClick={resetarParaOriginal}
                    title="Remover arquivo e restaurar capa original"
                    className="absolute right-3 p-1 rounded-lg text-red-500 hover:bg-red-500/10 transition-colors"
                  >
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea 
              ...
              onChange={handleChange}
              placeholder="Descreva aqui o cronograma de leituras do mês..."
